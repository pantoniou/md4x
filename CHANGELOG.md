...
- **Progressive line-diff updates (`md4x_stream_render`)**: For terminals that update the active region in place, returns an `MD4X_STREAM_UPDATE` (`backtrack` lines to erase, replacement `content`, and `freeze` count of newly-permanent lines) instead of append-only output. The active region is rendered per `opts.heal` and diffed line by line, so typically only the last line(s) change; applying every update to a virtual screen reconstructs the one-shot render exactly. CLI `--stream-progressive` reads input incrementally and drives this live: on a terminal it updates the active region in place with cursor control (tracking the on-screen row count so it never over-backtracks), and writes the final reconstructed screen otherwise. See `test/stream-demo.sh`.
