...
- **Progressive line-diff updates (`md4x_stream_render`)**: For terminals that update the active region in place, returns an `MD4X_STREAM_UPDATE` (`backtrack` lines to erase, replacement `content`, and `freeze` count of newly-permanent lines) instead of append-only output. The active region is rendered unhealed and diffed line by line, so typically only the last line(s) change; applying every update to a virtual screen reconstructs the one-shot render exactly.
