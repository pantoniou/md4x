...
Each call appends input, re-renders the active region (healed per `opts.heal` — with healing on, dangling markers in the in-progress tail show closed), and diffs it against the previous render line by line. Committed/frozen lines always use the **unhealed truth** and are frozen only where healing did not change them, so committed output stays stable. Apply the update to a virtual terminal as: move the cursor up `backtrack` lines, clear to end of screen, then print `content`. Usually only the last line or two change (small `backtrack`); a table reflow may change the whole active region. `freeze` reports how many top lines just reached a safe sync point and are now permanent (future `backtrack` never exceeds the still-mutable line count). Applying every update to a virtual screen reconstructs exactly the one-shot render.

The `md4x` CLI drives this via `--format=ansi --stream-progressive`: it reads input incrementally and, when stdout is a terminal, applies each update with cursor control (move up `backtrack` lines, clear to end, reprint), tracking the on-screen row count so it never moves up past it. When stdout is not a terminal it writes the final reconstructed screen. See `test/stream-demo.sh`.
